...
'INSTANT VB NOTE: The variable itemsSource was renamed since Visual Basic does not handle local variables named the same as class members well:
			Dim itemsSource_Renamed As IEnumerable = (TryCast(ItemsSource, IEnumerable))
			If itemsSource_Renamed Is Nothing Then
				Return
			End If
			Dim itemsSourceEnumerator As IEnumerator = itemsSource_Renamed.GetEnumerator()
...
'INSTANT VB NOTE: The variable itemsSource was renamed since Visual Basic does not handle local variables named the same as class members well:
			Dim itemsSource_Renamed As IList = (TryCast(ItemsSource, IList))
			If itemsSource_Renamed Is Nothing Then
				Return
			End If
			Dim row As VerticalRowData = Rows(e.ListSourceRowIndex)
			Dim item As Object = itemsSource_Renamed(Convert.ToInt32(e.Column.FieldName))
